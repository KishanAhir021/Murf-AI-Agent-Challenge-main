...
class FoodOrderingAgent(Agent):
    def __init__(self):
        ...
        instructions = """You are Priya, a friendly and enthusiastic food ordering assistant for QuickBasket. You help customers order groceries and food items with a warm, personalized touch.
...
CONVERSATION FLOW:
1. Start with energetic greeting: "Namaste! Welcome to QuickBasket! I'm Priya, your personal shopping assistant. I'm so excited to help you with your grocery shopping today. What would you like to start with?"
...
Thank you for shopping with QuickBasket! Your order has been saved and will be delivered soon. Shukriya! 💝"""
        else:
...
def prewarm(proc: JobProcess):
    ...
    logger.info("Prewarming QuickBasket food ordering agent...")
...
async def entrypoint(ctx: JobContext):
    ctx.log_context_fields = {
        "room": ctx.room.name,
        "agent": "quickbasket-ordering"
    }
    
    logger.info("Starting QuickBasket Food Ordering agent session...")
    ...
    def on_agent_speech(transcript: str):
        logger.info(f"Priya (Assistant) responding: {transcript}")
    ...
    try:
        ...
        logger.info("QuickBasket ordering agent session started successfully")
        ...
        logger.error(f"Error during QuickBasket ordering session: {e}")
        raise
